...
def annotations2motion(num_joints, annotations, scale=1.0, smooth=True, visibility=False, mean_height=None, limit=-1, no_wrap=False):
    ...
    for annotation in annotations[:max(limit, len(annotations))]:
        ...
        if len(keypoint) == 51:
            ...
            coco = np.array(keypoint).reshape((-1, 3)) # by WS
        if visibility:
            coco = coco[:, :3]
        else:
            coco = coco[:, :2]
        nose = coco[0]
        right_shoulder = coco[2]# coco[6]
        right_elbow = coco[4]# coco[8]
        right_wrist = coco[6]
        left_shoulder = coco[1]
        left_elbow = coco[3]
        left_wrist = coco[5]
        right_hip = coco[8]
        right_knee = coco[10]
...
def csv2annotations(csv_path):
    #jointDict = convert(csv_path)
    ...
    for idx, line in enumerate(lines):
        ...
        objects['bbox'] = [0.0, 0.0, 0.0, 0.0]
        objects['score'] = 0.9
        kp = list()
        kp_tp = list([float(elt) for elt in line.rstrip("\n").split(",")[1:]])
        for num, elt in enumerate(kp_tp):
            kp.append(elt)
            if num % 2 == 1:
                kp.append(1)
        objects['keypoints'] = kp
        objects['area'] = 18000.0
        objects['b_score'] = 0.97
        objects['object_id'] = 0
...
def cocopose2motion(num_joints, json_dir, scale=1.0, smooth=True, visibility=False, mean_height=None, limit=-1):
    ...
    annotations = csv2annotations(json_dir)
